...
app.post('/clarity_upload', cors({ origin: true}), (req, res) => {
  if(req.body != {}) {
    ...
    const values = [{
      timestamp: decodedEvent.timestamp, raw_event: JSON.stringify(rawEvent), decoded_event: JSON.stringify(decodedEvent), user_id: decodedEvent.envelope.userId
    }]
